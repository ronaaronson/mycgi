...
class Form(dict):
    ...
    def __init__(
        ...
        if environ["REQUEST_METHOD"] in ("POST", "PUT"):
            ...
            if environ["CONTENT_TYPE"][0:16] == "application/json":
                ...
                for k, v in d.items():
                    self[k] = (
                        [Field(k, None, value) for value in v]
                        if isinstance(v, list)
                        else Field(k, None, v)
                    )
            else:
                headers = {
                    "Content-Type": cast(str, environ.get("CONTENT_TYPE")).encode()
                }
                if "CONTENT_LENGTH" in environ:
                    headers["Content-Length"] = cast(
                        str, environ["CONTENT_LENGTH"]
                    ).encode()
                python_multipart.parse_form(headers, fp, self._on_field, self._on_file)
        else:
            # GET or HEAD request
            for k, v in parse_qs(
                environ.get("QUERY_STRING", ""), keep_blank_values=keep_blank_values
            ).items():
                for value in v:
    ...
    def _add_field(
        ...
        list_or_field = self.setdefault(name, form_value)
        if list_or_field is not form_value:
            # The key already exists:
            if isinstance(list_or_field, list):
                list_or_field.append(form_value)
            else:
                self[name] = [list_or_field, form_value]
